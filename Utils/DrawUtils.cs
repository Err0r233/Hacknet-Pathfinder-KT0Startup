...
namespace KT0Mods.Utils
{
    public class DrawUtils
    {
        ...
        public static void ClipLineSegmentsInRect(Rectangle bound, Vector2 left, Vector2 right, out Vector2 a, out Vector2 b)
        {
            ...
            {
                aModified = true;
                a = new Vector2(bound.X, Utils.Ratio(left.X, left.Y, right.X, right.Y, bound.X));
            }
            if (a.X > (bound.X + bound.Width))
            {
                ...
                aModified = true;
                a = new Vector2(bound.X + bound.Width, Utils.Ratio(left.X, left.Y, right.X, right.Y, bound.X + bound.Width));
            }
            if (a.Y < bound.Y)
            {
                if (aModified)
                ...
                aModified = true;
                a = new Vector2(Utils.Ratio(left.Y, left.X, right.Y, right.X, bound.Y), bound.Y);
            }
            if (a.Y > (bound.Y + bound.Height))
            {
                ...
                a = new Vector2(Utils.Ratio(left.Y, left.X, right.Y, right.X, bound.Y + bound.Height), bound.Y + bound.Height);
            }
            if (b.X < bound.X)
            {
                bModified = true;
                b = new Vector2(bound.X, Utils.Ratio(left.X, left.Y, right.X, right.Y, bound.X));
            }
            if (b.X > (bound.X + bound.Width))
            {
                ...
                bModified = true;
                b = new Vector2(bound.X + bound.Width, Utils.Ratio(left.X, left.Y, right.X, right.Y, bound.X + bound.Width));
            }
            if (b.Y < bound.Y)
            {
                if (bModified)
                ...
                bModified = true;
                b = new Vector2(Utils.Ratio(left.Y, left.X, right.Y, right.X, bound.Y), bound.Y);
            }
            if (b.Y > (bound.Y + bound.Height))
            {
                ...
                b = new Vector2(Utils.Ratio(left.Y, left.X, right.Y, right.X, bound.Y + bound.Height), bound.Y + bound.Height);
            }
        }
